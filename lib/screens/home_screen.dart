    ...
    try {
      ...
      switch (_currentIndex) {
        ...
        case 1: // Filmler
          ...
          break;
        case 2: // Diziler
          // Geçici olarak boş liste döndür
          _items = [];
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Diziler özelliği şu anda bakım modunda')),
          );
          break;
      }
    } catch (e) {
              ...
              : GridView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return Card(
                      ...
                      child: InkWell(
                        onTap: () {
                          // Diziler için özel işlem
                          if (item.streamType == 'series') {
                            ScaffoldMessenger.of(context).showSnackBar(
                              const SnackBar(content: Text('Dizi bölümleri henüz desteklenmiyor')),
                            );
                            return;
                          }
                          
                          // Filmler için geçici olarak devre dışı bırak
                          if (item.streamType == 'movie') {
                            ScaffoldMessenger.of(context).showSnackBar(
                              const SnackBar(content: Text('Film oynatma özelliği şu anda bakım modunda')),
                            );
                            return;
                          }
                          ...
                          if (item.streamUrl != null && item.streamUrl!.isNotEmpty) {
                            ...
                            ScaffoldMessenger.of(context).showSnackBar(
                              const SnackBar(content: Text('Stream URL bulunamadı')),
                            );
                          }
                        },
                        child: Stack(
