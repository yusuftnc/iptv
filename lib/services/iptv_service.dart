...
class IptvService {
  ...
  Future<List<Map<String, dynamic>>> getSeries() async {
    try {
      ...
      print('Series API URL: $uri');
      final response = await http.get(uri);
      print('Series API response status: ${response.statusCode}');

      if (response.statusCode == 200) {
        print('Series API response body: ${response.body.substring(0, min(500, response.body.length))}...');
        final List<dynamic> data = json.decode(response.body);
        print('Series data length: ${data.length}');
        
        // Debug için ilk dizi verisini yazdır
        if (data.isNotEmpty) {
          print('First series data: ${data[0]}');
        } else {
